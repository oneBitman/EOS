...
#include "Lib_Flash.h"
#include "Functions.h"
#include "Variables.h" //check variables file for various customization definitions
...
void setup() {
  ...
  pulseOut(false);
  realBPM = 96;  //the BPM set at startup
  //
  startMIDIinterface();
  initTM1638();
  reset();
  splashScreen();  //random intro animation
...
void loop() {
  static byte updateCounter = 1;  // Counter for limiting screen updates and key scanning routine

  // // Handle complete SysEx message
  // if (sysExCompleteFlag) {
  //   sysExCompleteFlag = false;
  //   processSysExChunk((byte*)sysExBuffer, sysExBufferIndex);
  //   sysExBufferIndex = 0;  // Reset buffer after processing
  // }

  // // Handle SysEx chunk completion
  // if (sysExChunkCompleteFlag) {
  //   sysExChunkCompleteFlag = false;  // Reset flag after processing
  //   processSysExChunk((byte*)sysExBuffer, sysExBufferIndex);
  //   sysExBufferIndex = 0;  // Reset buffer for next chunk
  // }

  // // Handle SysEx overflow errors
  // if (sysExOverflowFlag) {
  //   sysExOverflowFlag = false;
  //   showErrorMsg(20);  // Handle overflow error
  // }

  // Regular operations
  if (!receivingExternalSysEx) {
    // 1. Tick the sequencer
    if (!globalData.midiClockInternal) {
      doTickSequencer();
    }

    // 2. Check pattern stream
    checkPatternStream();

    // 3. Read button inputs
    readButtons();

    // 4. Handle MIDI input
    handleMIDIInput();

    // 5. Manage screen and interface updates
    updateCounter++;
    if (updateCounter >= 8) {
      if (updateScreen) {
        updateScreen = false;
    ...
    // Reset transpose editing if no longer holding a step
    ...
    // Reset the counter when it reaches the max value
    if (freeRunningCounter >= 3500) {
      freeRunningCounter = 0;
    }
    // Quantize the counter to 0-15
    quantizedValue = freeRunningCounter / (3500 / 16);

  } else {
    // During SysEx reception, prioritize handling incoming MIDI input
    handleMIDIInput();
  }
}
